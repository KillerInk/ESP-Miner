...
void BAP_send_request(bap_parameter_t param) {
    ...
    switch (param) {
        case BAP_PARAM_SYSTEM_INFO:
            ...
            char port_str[6];
            snprintf(port_str, sizeof(port_str),"%u", POOL_MODULE.pools[POOL_MODULE.active_pool].port);
            BAP_send_message(BAP_CMD_RES, "pool", POOL_MODULE.pools[POOL_MODULE.active_pool].url);
            BAP_send_message(BAP_CMD_RES, "poolPort", port_str);
            BAP_send_message(BAP_CMD_RES, "poolUser", POOL_MODULE.pools[POOL_MODULE.active_pool].user);
            break;
