...
int stratum_submit_share(char * jobid, char * extranonce2, uint32_t ntime, uint32_t nonce, uint32_t version)
{
    ...
    int ret = STRATUM_V1_submit_share(sock, send_uid++, user, jobid, extranonce2, ntime, nonce, version);
   ...
   while (1) {
       ...
       switch (stratum_api_v1_message.method) {
           case MINING_NOTIFY:
               ...
               if (create_jobs_task_handle != NULL) { // Removed
