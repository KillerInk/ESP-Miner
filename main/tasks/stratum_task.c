...
void stratum_primary_heartbeat(void * pvParameters)
{
    ...
    while (1) {
        ...
        STRATUM_V1_authorize(hb_sock, send_uid++, POOL_MODULE.pool_user, POOL_MODULE.pool_pass);
...
void stratum_task(void * pvParameters)
{
    ...
    extranonce_subscribe =
        POOL_MODULE.is_using_fallback ? POOL_MODULE.fallback_pool_extranonce_subscribe : POOL_MODULE.pool_extranonce_subscribe;
    ...
    for (;;) {
        ...
        case STRATUM_STATE_PROCESS_SHARES:
            handle_process_shares();
            break;
        case STRATUM_STATE_ERROR_RETRY:
            handle_error_retry();
            break;
        case STRATUM_STATE_SWITCH_FALLBACK:
            handle_switch_fallback();
            break;
        case STRATUM_STATE_SHUTDOWN:
            handle_shutdown();
            break;
        }
    }
