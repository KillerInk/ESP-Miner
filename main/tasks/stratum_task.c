...
void handle_process_shares(void)
{
    ...
    case MINING_NOTIFY:
        ...
        stratum_api_v1_message.mining_notification->job_difficulty = POOL_MODULE.pools[POOL_MODULE.active_pool].difficulty;
        ...
        decode_mining_notification(stratum_api_v1_message.mining_notification, stratum_api_v1_message.extranonce_str,
                                   stratum_api_v1_message.extranonce_2_len);
        break;

    case MINING_SET_DIFFICULTY:
        ESP_LOGI(TAG, "Set pool difficulty: %ld", stratum_api_v1_message.new_difficulty);
        POOL_MODULE.pools[POOL_MODULE.active_pool].difficulty = stratum_api_v1_message.new_difficulty;
        break;
    ...
    case STRATUM_RESULT_SUBSCRIBE:
        ...
        ESP_LOGI(TAG, "Set extranonce: %s, extranonce_2_len: %d", stratum_api_v1_message.extranonce_str,
                 stratum_api_v1_message.extranonce_2_len);
    ...
    case STRATUM_RESULT_SETUP:
        ...
        if (stratum_api_v1_message.response_success) {
            ...
            if (stratum_api_v1_message.message_id == authorize_message_id) {
                STRATUM_V1_suggest_difficulty(POOL_MODULE.pools[POOL_MODULE.active_pool].sock, send_uid++, difficulty);
            }
            if (extranonce_subscribe) {
                STRATUM_V1_extranonce_subscribe(POOL_MODULE.pools[POOL_MODULE.active_pool].sock, send_uid++);
            }
        } else {
